                    ...
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                        ...
                        <div class="flex gap-2 mt-2 md:mt-0">
                            <x-ui.button variant="secondary" size="sm"
                                href="{{ route('paciente.citas.reprogramar.index') }}">Reprogramar</x-ui.button>
                            <x-ui.button variant="warning" size="sm"
                                href="{{ route('paciente.citas.cancelar.index') }}">Cancelar</x-ui.button>
                        </div>
                    </div>
                </div>
            @else
                <div class="bg-neutral-50 text-center py-6 border border-neutral-200 rounded-[var(--radius)]">
                    ...
                    <div class="mt-3">
                        <x-ui.button variant="primary" size="sm" href="{{ route('paciente.citas.create') }}">Agendar
                            cita</x-ui.button>
                    </div>
                </div>
            @endif
                    ...
                    @php
                        $acciones = [
                            ...
                            [
                                'icono' => '⛔',
                                'texto' => 'Cancelar cita',
                                'ruta' => route('paciente.citas.cancelar.index'),
                            ],
                            [
                                'icono' => '📝',
                                'texto' => 'Modificar cita',
                                'ruta' => route('paciente.citas.reprogramar.index'),
                            ],
                    ...
                    @foreach ($acciones as $a)
                        <a href="{{ $a['ruta'] }}"
                            ...
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <div class="text-sm font-medium text-neutral-900 group-hover:text-rose-700">
                                    {{ $a['texto'] }}</div>
                            </div>
                        </a>
                    @endforeach
                                    ...
                                    <tr class="hover:bg-neutral-50 transition">
                                        <td class="px-4 py-2">
                                            ...
                                            <span class="md:hidden inline-block">
                                                {{ $appointment->fecha_hora_inicio->locale('es')->translatedFormat('d/m, h:i A') }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-2">
