  ...
  <div class="space-y-8">
    <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <x-ui.button :href="route('medico.pacientes.index')" variant="primary">
          Mis pacientes
        </x-ui.button>
      ...
      <x-ui.card class="xl:col-span-2" title="Agenda del día" subtitle="Citas confirmadas para hoy">
        <div class="space-y-4">
          @forelse ($todayAppointments as $appointment)
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border border-neutral-200 rounded-2xl px-4 py-3 hover:border-primary-200 transition">
              <div class="flex items-center gap-4">
                <div class="text-center">
                  <p class="text-xl font-semibold text-neutral-900">{{ optional($appointment->fecha_hora_inicio)->format('H:i') }}</p>
                  <p class="text-xs text-neutral-500">{{ optional($appointment->fecha_hora_fin)->format('H:i') }}</p>
                </div>
                <div>
                  <p class="text-sm font-semibold text-neutral-900">
                    {{ optional($appointment->paciente)->nombres }} {{ optional($appointment->paciente)->apellidos }}
                  </p>
        ...
        <x-ui.card title="Indicadores del mes" subtitle="Con base en tus citas agendadas">
          <div class="space-y-4">
            ...
            <div class="w-full bg-neutral-200 rounded-full h-2">
              <div class="h-2 rounded-full bg-[var(--color-primary-500)]" style="width: {{ $productivity }}%"></div>
            </div>
